...
class CEP
  ...
  WEB_SERVICE_REPUBLICA_VIRTUAL_URL = 'http://cep.republicavirtual.com.br/web_cep.php?formato=query_string&cep='.freeze
  ...
  def self.busca(numero)
    ...
    response = Net::HTTP.Proxy(proxy_addr, proxy_port).get_response(URI.parse("#{WEB_SERVICE_REPUBLICA_VIRTUAL_URL}#{cep}"))
